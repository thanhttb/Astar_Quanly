                ...
                <div class="row">
                        ...
                        <div class="portlet box red">
                            <div class="portlet-title">
                                ...
                                <ul class="nav nav-tabs">
                                     <?php $tabIndex =1 ?>
                                    @foreach($classLearning as $class)
                                        <li>
                                            <a href="#portlet_tab_{{$tabIndex}}" data-toggle="tab" aria-expanded="false">{{$class['name']}}</a>
                                        </li>
                                        <?php $tabIndex++; ?>
                                    @endforeach
                                </ul>
                            ...
                            <div class="portlet-body">
                                <div class="tab-content">
                                    ...
                                    <?php $tabIndex = 1; ?>
                                    @foreach($classLearning as $class)
                                        <div class="tab-pane" id="portlet_tab_{{$tabIndex}}">
                                            <table id="atd{{$tabIndex}}" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                                <thead>
                                                    <tr class="heading">
                                                        <th>Ngày học</th>
                                                        <th>Giáo viên</th>
                                                ...
                                                <tbody>
                                                    @foreach($class['atd'] as $atd)
                                                        <tr>
                                                            <td>{{date('Y/m/d',strtotime($atd['start_time']))}}</td>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="portlet solid grey-cascade">
                            <div class="portlet-title">
                                <div class="caption">
                                    <i class="fa fa-gift"></i>Học phí tháng {{$period}}</div>
                                <div class="actions">
                            ...
                            <div class="portlet-body"> 
                                <table class="table table-bordered" cellspacing="0" width="100%">
                                    ...
                                    <tbody>
                                           @foreach($classLearning as $val)
                                             <tr>
                                                 <td>{{$val['name']}}</td>
                                                 <td>{{$val['lessonCount']}}</td>
                                                 <td>-{{asVnd($val['netTuition'])}}</td>
                                                 <td>{{$val['discount']}}%</td>
                                                 <td>{{asVnd($val['lastBalance'])}}</td>
                                                 <td>{{asVnd($val['otherFee'])}}</td>
                                                 <td>{{asVnd($val['otherFee'] + $val['tuitionAfterDiscount'])}}</td>
                                                 <td>{{asVnd($val['recieved'])}}</td>
                                                 <td>{{asVnd($val['balance'])}}</td>
                                             </tr>
                                           @endforeach
                                    </tbody>
                                </table>
...
<div class="col-md-12">
    ...
    <form action="#" method="POST" >
        ...
        <tbody id="body-transaction">
            ...
            @foreach($stdTransaction as $transaction)
                
                <tr>
                    <td>
                        @if($transaction['amount'] < 0 && is_null($transaction['rel_id']))
                            <input type="checkbox" name="id[{{$transaction['id']}}]" value= "{{$transaction['id']}}">
                        @endif
                    ...
                    <td>
                        @if($transaction['amount'] < 0)
                            {{$transaction['lessonInfo']['start_time']}}
                        @endif
                    </td>
                    @if($transaction['day'] < date('Y-m-d') && is_null($transaction['rel_id']) && $transaction['amount']<0)                    
                      <td class="bg-red-intense bg-font-red-intense" >{{$transaction['amount']}}</td>
                    @else <td>{{asVnd($transaction['amount'])}}</td>
                    @endif
                    <td>{{asVnd($transaction['balance'])}}</td>
